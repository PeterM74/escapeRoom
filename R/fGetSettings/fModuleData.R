...
fModuleData <- function(Settings) {
  ...
  list(
    ...
    'DMD' = list(Print = .PrintModuleMessage("DMD", Settings),
                 Hint = paste("One of the diamonds must be clearly different from the others.",
                              "Maybe a combination of variables will reveal the fake diamond."),
                 Extrahint = paste("One method of using a combination of variables to determine",
                                   "trends/usual patterns in data is regression. Perhaps a fake diamond",
                                   "would be cheaper than a real one?"),
                 Submit = "1713",
    ...
    'SSS' = list(Print = .PrintModuleMessage("SSS", Settings),
                 ...
                 Success = "Correct! Module unlocked. Key: !.",
