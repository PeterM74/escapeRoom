...
#' `GenericHelpResponse` is what is output when 'help' is entered in the terminal, while 
#' `SpecificCommandHelpers` contains the specific outputs when 'help <command>' is run.
...
fHelpCommand <- function(Settings) {
  ...
  SpecificCommandHelpers <- c("abort" = paste("Aborts the countdown for the vessel disbursement.", 
                                              "Requires a ", stringr::str_length(Settings$AbortPasscode),
                                              "-character passcode."),
                              "deploy" = "Initiates countdown sequence. No options.",
                              "help" = "Well now you are just being silly.",
                              "hello" = "??? It's just hello",
                              "module" = paste0("Runs module of terminal.\n",
                                                "First argument specifies module code.\n",
